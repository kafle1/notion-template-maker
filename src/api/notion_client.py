...
class NotionClient:
    ...
    @classmethod
    def from_oauth_token_data(
        cls, token_data: Dict[str, Any], notion_version: str = "2022-06-28"
    ) -> "NotionClient":
        """
        Create a NotionClient instance from OAuth token data.
        ...
        if not access_token:
            raise ValueError("Missing access_token in token data")

        return cls(access_token, notion_version)

    def get_oauth_user_info(self) -> Dict[str, Any]:
        """
        ...
        """
        # This would typically come from the OAuth token data
    ...
    def validate_oauth_token(self) -> bool:
        """
        Validate that the OAuth token is still valid by making a test request.
    ...
    def refresh_oauth_token(
        ...
        oauth_service = NotionOAuthService(client_id, client_secret, "")
        return oauth_service.refresh_token(refresh_token)
